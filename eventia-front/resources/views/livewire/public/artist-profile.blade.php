    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100/50 overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    <div>
                        <span class="text-[10px] uppercase tracking-widest text-text-secondary font-bold">Género
                            Musical</span>
                        <p class="text-sm font-bold text-text-main mt-1 italic">{{ $artist->genero_musical }}</p>
                    </div>
                    <div>
                        <span class="text-[10px] uppercase tracking-widest text-text-secondary font-bold">Tipo de
                            Formación</span>
                        <p class="text-sm font-bold text-text-main mt-1">{{ $artist->tipo }}</p>
                    </div>
                    <div>
                        <span class="text-[10px] uppercase tracking-widest text-text-secondary font-bold">Precio de
                            Referencia</span>
            ...
            <div class="bg-gradient-to-br from-secondary/10 to-accent/10 rounded-3xl p-6 border border-secondary/10">
                ...
                <p class="text-xs text-text-secondary leading-relaxed">Este perfil pertenece a un artista o banda
                    registrada y cuya trayectoria ha sido verificada por Eventia.</p>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div
                ...
                <div
                    class="absolute top-0 right-0 w-48 h-48 bg-secondary/5 rounded-full -mr-24 -mt-24 transition-transform group-hover:scale-110">
                </div>

                <div class="flex flex-col md:flex-row gap-8 items-center relative">
                    <div
                        class="w-32 h-32 rounded-3xl bg-gradient-to-br from-secondary to-accent p-1 shadow-xl shadow-secondary/20">
                        <div
                            ...
                            @if(isset($artist->img_logo) && $artist->img_logo)
                                <img src="{{ asset('profiles/artistas/' . $artist->img_logo) }}"
                                    alt="{{ $artist->nombre_artistico }}" class="w-full h-full object-cover">
                            @else
                                🎤
                            @endif
                        </div>
                    ...
                    <div class="flex-1 text-center md:text-left space-y-2">
                        <h2 class="text-4xl font-black text-text-main tracking-tight font-heading">
                            {{ $artist->nombre_artistico }}</h2>
                        <div class="flex items-center justify-center md:justify-start gap-4">
                            <span
                                class="px-3 py-1 bg-gray-50 text-[10px] font-bold text-text-secondary rounded-lg border border-gray-100">Artista
                                Profesional</span>
                ...
                <div class="mt-12">
                    <div class="flex items-center justify-between mb-8">
                        <div class="flex items-center gap-4">
                            ...
                            <h3 class="text-2xl font-black text-text-main font-heading tracking-tight">Presentaciones
                                Recientes</h3>
                        </div>
                        <span
                            class="text-xs font-bold text-accent bg-accent/10 px-4 py-1.5 rounded-full">{{ $artist->eventos->count() }}
                            Eventos</span>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        @empty
                            <div class="col-span-2 text-center py-10 bg-gray-50 rounded-2xl border border-gray-100">
                                ...
                                <p class="text-xs text-text-secondary mt-1">Este artista estará disponible próximamente en
                                    nuevos eventos.</p>
                            </div>
                        @endforelse
                    </div>
