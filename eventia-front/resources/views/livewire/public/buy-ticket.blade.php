<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="bg-white rounded-[40px] shadow-2xl overflow-hidden border border-gray-100/50 relative">
        ...
        <div class="relative h-80 overflow-hidden group">
            ...
            <div class="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1459749411177-042180ce673c?q=80&w=2070')] bg-cover bg-center mix-blend-overlay blur-[2px] scale-105 group-hover:scale-100 transition-transform duration-700"></div>
            
            <div class="relative h-full px-10 flex flex-col justify-center text-white">
                <div class="flex flex-col md:flex-row items-center gap-10">
                    <!-- Main Event Image -->
                    <div class="w-48 h-48 rounded-3xl bg-white/10 backdrop-blur-md p-2 shadow-2xl shadow-primary/20 flex-shrink-0">
                    ...
                    <div class="absolute top-8 right-8 flex flex-col gap-2">
                        <button class="p-4 bg-white/10 backdrop-blur-md rounded-2xl text-white hover:bg-primary transition-all duration-300 group/btn">
                        ...
                        <p class="text-[10px] font-bold text-center uppercase tracking-wider text-white/60 group-hover:text-white transition-colors">Recordatorio</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="p-8 sm:p-12 space-y-10">
            ...
            <!-- Step 1: Quantity -->
            <div class="bg-gray-50 border border-gray-100 rounded-[30px] p-8 flex flex-col md:flex-row items-center justify-between gap-6 hover:shadow-xl hover:shadow-gray-200/20 transition-all group">
                <div>
                    <h3 class="text-xl font-bold text-text-main font-heading mb-1">Cantidad de Entradas</h3>
                    <p class="text-text-secondary text-sm italic font-medium">Puedes añadir hasta un máximo de 5</p>
                </div>
                <div class="flex items-center gap-6 bg-white p-3 rounded-2xl shadow-sm border border-gray-100">
                    <button wire:click="decrement" class="w-12 h-12 rounded-xl bg-gray-50 hover:bg-gray-100 flex items-center justify-center text-text-main font-black text-xl transition-colors">-</button>
                    <span class="text-3xl font-black text-primary min-w-[40px] text-center">{{ $quantity }}</span>
                    <button wire:click="increment" class="w-12 h-12 rounded-xl bg-primary text-white hover:opacity-90 flex items-center justify-center text-text-main font-black text-xl transition-colors shadow-lg shadow-primary/20">+</button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-4">
                        ...
                        <label wire:click="$set('category', '{{ $label }}')" class="flex items-center justify-between p-5 rounded-2xl border-2 {{ $category === $label ? 'border-primary bg-primary/5 shadow-md shadow-primary/5' : 'border-gray-100 bg-white hover:border-gray-200' }} cursor-pointer transition-all group relative overflow-hidden">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="w-5 h-5 rounded-full border-2 border-gray-200 flex items-center justify-center transition-colors {{ $category === $label ? 'border-primary' : '' }}">
                                    @if($category === $label) <div class="w-2.5 h-2.5 rounded-full bg-primary"></div> @endif
                                </div>
                ...
                <div class="bg-gradient-to-br from-text-main to-gray-800 rounded-[35px] p-10 text-white flex flex-col justify-between relative overflow-hidden shadow-2xl">
                    ...
                    <button wire:click="buy" class="w-full mt-10 bg-gradient-to-r from-primary via-secondary to-accent text-white font-black py-5 rounded-2xl shadow-xl shadow-primary/30 hover:shadow-primary/50 transition-all transform hover:-translate-y-1 hover:scale-[1.02] flex items-center justify-center gap-3 active:scale-95 group">
                        Confirmar y Pagar
                        <svg class="w-6 h-6 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                    </button>
                </div>
            </div>
        </div>
